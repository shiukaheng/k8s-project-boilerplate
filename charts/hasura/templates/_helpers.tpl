...
{{- else if (.Values.metadataPostgres).enabled -}}
  {{- $password := coalesce (.Values.metadataPostgres).password "hasura" -}}
  {{- printf "postgres://postgres:%s@%s-metadata-postgres:5432/postgres" $password (include "common.prefix" .) -}}
{{- else -}}
